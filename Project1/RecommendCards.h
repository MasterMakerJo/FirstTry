...
public:
	...
	int Help0Pass(fDdz * pDdz, int playerType);     //推荐出牌应对两家PASS，出单牌
	...
	int Help8LinkDouble(struct fDdz * pDdz);		//H08 推荐出牌应对双顺  连对
	int Help9LinkThree(struct fDdz * pDdz);			//H09 推荐出牌应对三顺
	int Help10LinkThreeSingle(struct fDdz * pDdz);	//H1001 推荐出牌应对三顺带单  飞机
	...
	int GetPlayerType(fDdz *pDdz);// 获得玩家类型
	CardsInfo* CInfo;
	int iPlaCount;			//本次模拟出牌生成器产生的多种出牌可行解数量
	SplitCard *pSplitCards;//拆牌类变量
	PokerModelAnalysis *pMoldel;
	int OtherPlayersCards(fDdz* pDdz, Ddz *Ddz);//其他玩家手牌
	int* pCardsOnHand;
	char cAI;//记录AI方位
	void HelpTakeOff(fDdz* pDdz, int num);	//M07 从手牌模拟移除一组牌
