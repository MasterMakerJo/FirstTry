...
void MonteCarloSearchEngine::Init()
{
	...
	{
	case 'A'://如果我是A方位，则我的下家是B方位，我的上家是C方位，按顺时针方向
		pUpDdzBoard->cDir = 'C';
		pDownDdzBoard->cDir = 'B';
		break;
	case 'B'://如果我是B方位，则我的上家是A方位，我的下家是C方位，同样顺时针
		pUpDdzBoard->cDir = 'A';
		pDownDdzBoard->cDir = 'C';
		break;
	case 'C'://如果我是C方位，则我的下家是A方位，我的上家是B方位
		pUpDdzBoard->cDir = 'B';
		pDownDdzBoard->cDir = 'A';
		break;
	}
	pUpDdzBoard->cLastPlay = pDownDdzBoard->cDir;//就是讲真实的牌局信息赋给模拟的牌局信息
	...
	//初始化模拟牌局信息
	pMyDdzBoard->iLastPassCount = pRealDdz->iLastPassCount;//效果同上，将真实信息交递给模拟信息
	...
	//记录AI最后一次有效桌面信息
	memcpy(pMyDdzBoard->iOnHand, pRealDdz->iOnHand, sizeof(pRealDdz->iOnHand));//从真实信息所指的内存地址的起始位置开始拷贝相应字节到模拟信息所指的内存地址的起始位置中
	int i;
	for (i = 0; pRealDdz->iOnTable[i][0] != -2; i++);//已出牌数组
	if (pRealDdz->iLastPassCount == 1)//连续PASS数，一家pass取1，两家pass取2，没人pass就取0
	{
		pMyDdzBoard->iLastOnTable = pRealDdz->iOnTable[i - 2];//已出牌数组，为什么－2，pass了一个人
	}
	else
	{
		//如果第一次出牌
...
void MonteCarloSearchEngine::SearchAGoodMove(Ddz *pDdz)
{
	...
	iNodesCount = pRecomCards->HelpPla(pMyDdzBoard,0);//可行解只有这一个就是它
	if (iNodesCount == 1)
	{
