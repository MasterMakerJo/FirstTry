...
int RecommendCards::InitPlaGenerate(fDdz* pDdz)
{
	...
	iPlaCount = 0;
	pDdz->iPlaCount = 0;
	iHandNum = MinHandNum(pDdz->iOnHand);//最少手牌数？
	isSim = false;
...
int RecommendCards::HelpPla(struct fDdz * pDdz, bool IsSim) //推荐出牌
{
	int iPass[3] = { -2, -1, -1 };
	bool isPassed = false;
...
int RecommendCards::Help0Pass(fDdz * pDdz, int playerType)//推荐出牌应对两家pass
{
	...
	//如果首出
	if ((cardsCount == 17 && playerType != 1) || cardsCount == 20)//如果是地主或者手上17张牌一张都没有出的时候
	{

        //可不可以将这些if合并，还是说调用了后面的函数issim的值在改变
		if (isSim)
			Help3SingleSim(pDdz);
		if (!isSim)
			Help3Single(pDdz);
